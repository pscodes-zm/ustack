...
export const getBtcPrice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      return { priceUsd, priceZmw, cachedAt: new Date().toISOString(), fresh: true };
    } catch {
      ...
      if (last) return { priceUsd: Number(last.price_usd), priceZmw: Number(last.price_zmw), cachedAt: last.fetched_at, fresh: false };
      return { priceUsd: 105_000, priceZmw: 105_000 * ZMW_PER_USD, cachedAt: new Date().toISOString(), fresh: false };
    }
  });
